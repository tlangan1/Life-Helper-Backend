...
- `Express Server Setup`
  ...
  - Open the "Production-Clones/Express-Server" directory in bash and execute the following command
    ...
    ```
- `Life Helper Setup`
  - Make sure vite.config.js points to port 3002
...
- Here is a diagram of the relationship between users and tasks.
  ...
  - If a user is unable to complete a given task and it needs to get reassigned to another user then there would be at least two rows in task_user associated with that task.
  - A task can be reassigned only if it is in the started or paused state.
    ...
    - Reassignment of a task is consists of nothing more than the insertion of a task_user row for the new user
