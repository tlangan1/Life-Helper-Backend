...
./upgrade_or_create_environment.sh $1 $2 || { echo "Error during environment setup"; exit 1; }
...
./load_existing_test_objects.sh $1 $2 || { echo "Error loading existing test objects"; exit 1; }

./run_existing_tests.sh $1 $2 || { echo "Error running existing tests"; exit 1; }

if [ "$3" = "run_new_tests" ]; then
    echo "Running new tests"
    ./load_new_test_objects.sh $1 $2 || { echo "Error loading new test objects"; exit 1; }
    ./run_new_tests.sh $1 $2 || { echo "Error running new tests"; exit 1; }
fi
